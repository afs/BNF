...
public class IndentedOutput
    //extends AWriterBase implements AWriter, Closeable
    //implements Closeable
{
    /** Stdout wrapped in an IndentedWriter - no line numbers */
    public static final IndentedOutput stdout = new IndentedOutput(System.out).setFlushOnNewline(true);
    /** Stderr wrapped in an IndentedWriter - no line numbers */
    public static final IndentedOutput stderr = new IndentedOutput(System.err).setFlushOnNewline(true);
    ...
    public IndentedOutput(OutputStream outStream) { this(outStream, false); }

    /** Construct a UTF8 IndentedWriter around an OutputStream */
    public IndentedOutput(OutputStream outStream, boolean withLineNumbers) {
        PrintStream printStream;
        if ( outStream instanceof PrintStream ps )
            this.out = ps;
        else
            this.out = new PrintStream(outStream, false, StandardCharsets.UTF_8);
    }
     ...
     */
    public static IndentedOutput clone(IndentedOutput other) {
        IndentedOutput dup = new IndentedOutput(other.out);
...
//    @Override
    ...
    protected IndentedOutput self() { return this; }
    ...
    public IndentedOutput setAbsoluteIndent(int x) { currentIndent = x; return self(); }

    public boolean hasLineNumbers() {
        return lineNumbers;
    }

    public IndentedOutput setLineNumbers(boolean lineNumbers) {
    ...
    public IndentedOutput setEndOfLineMarker(String marker)  {  endOfLineMarker = marker; return self(); }

    /** Flat mode - print without NL, for a more compact representation */
    public boolean inFlatMode()                     { return flatMode; }
    /** Flat mode - print without NL, for a more compact representation*/
    public IndentedOutput setFlatMode(boolean flatMode) {
     ...
     * This is set for {@link IndentedOutput#stdout} and {@link IndentedOutput#stderr}
     ...
     */
    public IndentedOutput setFlushOnNewline(boolean flushOnNewline) {
    ...
    public IndentedOutput setPadChar(char ch)       { this.padChar = ch; return self(); }
    ...
    public IndentedOutput setLinePrefix(String str) {
        this.linePrefix = str;
        return self();
    }
    ...
    public IndentedOutput setUnitIndent(int x) {
        unitIndent = x;
        return self();
    }
     ...
     */
    public IndentedOutput setNumberWidth(int widthOfNumbers) {
